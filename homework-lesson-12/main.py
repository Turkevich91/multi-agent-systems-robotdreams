...
def _ensure_report_saved(
    ...
    This is not the baseline homework workflow. The canonical Lesson 12 path is:
...
def main() -> None:
    ...
    print("Multi-Agent Research System (lesson 12 + Langfuse). Type 'exit' to quit.")
    print("-" * 60)
    ...
    while True:
        ...
        try:
            ...
            with langfuse_observed_run(user_input, thread_id) as lf_run:
                ...
                while interrupts:
                    interrupts = _resume_after_interrupt(interrupts, runtime_config, trace)

                _ensure_report_saved(user_input, runtime_config, trace)
                lf_run.output = {
                    "final_ai_text": trace.get("final_ai_text"),
                    "save_report_succeeded": trace.get("save_report_succeeded"),
                    "save_report_message": trace.get("save_report_message"),
                    "proposed_filename": trace.get("proposed_filename"),
                }
                if lf_run.trace_id:
