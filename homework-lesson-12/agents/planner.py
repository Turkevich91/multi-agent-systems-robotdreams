...
planner_agent = create_agent(
    ...
    system_prompt=load_system_prompt("planner"),
...
planner_fallback_agent = create_agent(
    model=build_chat_model(),
    tools=[web_search, knowledge_search],
    system_prompt=load_system_prompt("planner_fallback"),
    name="planner_fallback_agent",
)


def _runtime_config(runtime: ToolRuntime | None, *, recursion_limit: int) -> dict:
    config = dict(getattr(runtime, "config", {}) or {})
...
@tool
def plan(request: str, runtime: ToolRuntime = None) -> str:
    """Create a structured research plan for the user's request."""
    result = planner_agent.invoke(
        {"messages": [{"role": "user", "content": request}]},
        config=_runtime_config(runtime, recursion_limit=8),
    )
