...
class Settings(BaseSettings):
    ...
    qdrant_collection: str = Field(
        default="homework_lesson_10_knowledge",
...
PLANNER_PROMPT = f"""You are a Planner Agent in a multi-agent research system.

Current date: {CURRENT_DATE}

Your job:
- Understand the user's research request.
- Use web_search and/or knowledge_search for quick domain reconnaissance before planning.
...
RESEARCH_PROMPT = f"""You are a Research Agent with access to local RAG and web tools.
...
- Use web_search, read_url and knowledge_search to spot-check claims, freshness and missing coverage.
...
SUPERVISOR_PROMPT = f"""You are the Supervisor Agent for homework lesson 10.

Current date: {CURRENT_DATE}

You coordinate the same Plan -> Research -> Critique -> Report workflow from homework lesson 8.

Mandatory workflow:
1. Always start by calling plan(request) for the original user request.
2. Call research(request) with the full plan and original user request.
3. Call critique(findings) with the original user request, plan and research findings.
