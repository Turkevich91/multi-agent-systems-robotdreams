...
| `api.py` | FastAPI backend з SSE streaming і HITL endpoint. |
...
5. Людина підтверджує або додає власні criteria.
6. Expert Critic Panel виконує role-specific reviews.
7. Aggregator формує verdict.
8. Якщо потрібна доробка, Analyst робить revision.
9. Compiler генерує Markdown report і Mermaid diagrams.
10. Report зберігається в `output/`.
