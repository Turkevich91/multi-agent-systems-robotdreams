...
def ingest():
    ...
    try:
        ...
        raise RuntimeError(
            "Failed to build embeddings for the Qdrant index. Check embedding settings: "
            "EMBEDDING_MODEL, OPENAI_EMBEDDING_API_KEY and OPENAI_EMBEDDING_BASE_URL. "
            "The chat model can stay on LM Studio, but embeddings must use an endpoint "
            "that supports the embeddings API."
        ) from exc
    ...
    try:
        ...
        raise RuntimeError(
            "Failed to write vectors into Qdrant. Check that the Qdrant Docker "
            f"container is running and reachable at {settings.qdrant_url}."
        ) from exc
